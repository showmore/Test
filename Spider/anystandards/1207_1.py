...
def get_intro(soup):
	intro = ''
	try:
		intro = soup.find('div',{'class':'soft_intro'}).find('div').get_text().replace('/n',' '),replace('/t',' ')
	except:
		pass
	return intro
...
if __name__ == '__main__':
	...
	url = 'http://www.anystandards.com/industry/cjt/45004.html'
	# 设置浏览器请求头
	dcap = dict(DesiredCapabilities.PHANTOMJS)
	dcap["phantomjs.page.settings.userAgent"] = "Mozilla/5.0 (Linux; Android 5.1.1; Nexus 6 Build/LYZ28E) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/48.0.2564.23 Mobile Safari/537.36"
	...
	driver = webdriver.PhantomJS(executable_path="phantomjs.exe",desired_capabilities=dcap,service_args=service_args)
